...
function CampaignModuleView({moduleId,user,organization,members,locations,items,reload}:{moduleId:keyof typeof campaignModuleConfig;user:User;organization:Organization;members:Member[];locations:Headquarters[];items:CampaignRecord[];reload:()=>Promise<void>}){
...
function Dashboard({ session, profile }: { session: Session; profile: Profile }) {
  ...
  return <main className="app-shell" style={{"--navy":organization.primary_color,"--sun":organization.accent_color} as React.CSSProperties}>
    ...
    <div className="page module-stage" key={active}>
      ...
      {(Object.keys(campaignModuleConfig) as (keyof typeof campaignModuleConfig)[]).map(moduleId=>active===moduleId&&<CampaignModuleView key={moduleId} moduleId={moduleId} user={session.user} organization={organization} members={members} locations={headquarters} items={campaignRecords[moduleId]} reload={loadContext}/>)}
