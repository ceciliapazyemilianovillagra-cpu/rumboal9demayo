...
import { MobilityLiveMap } from "./mobility-live-map";
...
type MobilityTransfer={id:string;voter_id:string;voter_name:string;operation_date:string;driver_user_id:string;driver_name?:string|null;mobilizer_user_id?:string|null;mobilizer_name?:string|null;vehicle_id?:string|null;vehicle_plate?:string|null;driver_access_code?:string|null;destination_name:string;destination_address?:string|null;pickup_address?:string|null;status:"pendiente"|"buscada"|"en_destino"|"regresada";status_updated_at?:string|null;last_latitude?:number|null;last_longitude?:number|null};
function MobilityOperator({organization,members,locations,voters,vehicles,user}:{organization:Organization;members:Member[];locations:Headquarters[];voters:Voter[];vehicles:CampaignRecord[];user:User}){
 const [items,setItems]=useState<MobilityTransfer[]>([]),[events,setEvents]=useState<{id:string;transfer_id:string;status:string;created_at:string;actor_id:string}[]>([]),[open,setOpen]=useState(false),[message,setMessage]=useState(""),[filter,setFilter]=useState("todos"),[operationDate,setOperationDate]=useState(dateKey(new Date())),[selectedVehicle,setSelectedVehicle]=useState(""),[selectedDriver,setSelectedDriver]=useState(""),[selectedVoterIds,setSelectedVoterIds]=useState<string[]>([]),[dniSearch,setDniSearch]=useState(""),[assigning,setAssigning]=useState(false),[selectedTransfer,setSelectedTransfer]=useState<MobilityTransfer|null>(null),[editing,setEditing]=useState(false);
 const load=useCallback(async()=>{const [result,eventResult,routeResult]=await Promise.all([supabase.from("mobility_transfers").select("*").eq("organization_id",organization.id).eq("operation_date",operationDate).order("status_updated_at",{ascending:false}),supabase.from("mobility_events").select("*").eq("organization_id",organization.id).order("created_at",{ascending:false}).limit(200),supabase.from("mobility_driver_routes").select("*").eq("organization_id",organization.id).eq("operation_date",operationDate)]);const routeStops=((routeResult.data??[]) as {stops?:{transfer_id:string;status:"pendiente"|"buscada"|"en_destino"|"regresada";status_updated_at?:string;last_latitude?:number|null;last_longitude?:number|null}[] }[]).flatMap(route=>route.stops??[]);const updates=new Map(routeStops.map(stop=>[stop.transfer_id,stop]));setItems((result.error?[]:(result.data??[]) as MobilityTransfer[]).map(item=>({...item,...updates.get(item.id)})));setEvents(eventResult.error?[]:(eventResult.data??[]) as {id:string;transfer_id:string;status:string;created_at:string;actor_id:string}[]);},[organization.id,operationDate]);
 useEffect(()=>{void load();const timer=window.setInterval(()=>void load(),15000);return()=>window.clearInterval(timer);},[load]);
 async function assign(event:FormEvent<HTMLFormElement>){event.preventDefault();if(assigning)return;setAssigning(true);const form=event.currentTarget,data=new FormData(form),driver=members.find(item=>item.user_id===selectedDriver),vehicle=vehicles.find(item=>item.id===selectedVehicle);try{if(!selectedVoterIds.length||selectedVoterIds.length>20||!driver||!vehicle)throw new Error("Elegí un vehículo, su chofer y entre 1 y 20 personas.");const selectedVoters=selectedVoterIds.map(id=>voters.find(item=>String(item.id)===id)).filter(Boolean) as Voter[];const routeCode=`MOV-${crypto.randomUUID().replace(/-/g,"").slice(0,8).toUpperCase()}`,day=String(data.get("operation_date")),now=new Date().toISOString(),plate=String(data.get("vehicle_plate")||vehicle.contact_phone||"Patente a definir"),mobilizerId=String(data.get("mobilizer_user_id")||"");const records=selectedVoters.map(voter=>{const school=locations.find(location=>location.name.toLocaleLowerCase("es")===String(voter.polling_place||"").toLocaleLowerCase("es"));return {id:crypto.randomUUID(),organization_id:organization.id,operation_date:day,voter_id:String(voter.id),voter_dni:voter.dni,voter_name:voter.full_name,pickup_address:voter.address||"Dirección a confirmar",voter_phone:(voter.source_data?.telefono||voter.source_data?.phone||null),destination_id:school?String(school.id):null,destination_name:voter.polling_place||"Escuela a confirmar",destination_address:school?.address||null,driver_user_id:driver.user_id,driver_name:driver.profiles?.full_name||"Chofer",mobilizer_user_id:mobilizerId||null,mobilizer_name:members.find(item=>item.user_id===mobilizerId)?.profiles?.full_name||null,vehicle_id:vehicle.id,vehicle_plate:plate,driver_access_code:routeCode,status:"pendiente",status_updated_at:now,created_by:user.id};});const routeStops=records.map(record=>({transfer_id:record.id,dni:record.voter_dni,full_name:record.voter_name,pickup_address:record.pickup_address,voter_phone:record.voter_phone,destination_name:record.destination_name,destination_address:record.destination_address,status:"pendiente",status_updated_at:now,last_latitude:null,last_longitude:null}));const expiration=new Date(`${day}T23:59:59-03:00`).getTime()+12*60*60*1000;const routeResult=await supabase.from("mobility_driver_routes").insert({id:routeCode,organization_id:organization.id,operation_date:day,driver_name:driver.profiles?.full_name||"Chofer",vehicle_plate:plate,expires_at_ms:expiration,stops:routeStops,created_by:user.id});if(routeResult.error)throw routeResult.error;const {error}=await supabase.from("mobility_transfers").insert(records);if(error)throw error;await supabase.from("logistics_items").update({responsible_user_id:driver.user_id,contact_phone:plate}).eq("id",vehicle.id);form.reset();setSelectedVehicle("");setSelectedDriver("");setSelectedVoterIds([]);setDniSearch("");setOpen(false);setMessage(`${records.length} traslados asignados. Código del chofer: ${routeCode}.`);await load();}catch(cause){setMessage(cause instanceof Error?cause.message:"No se pudieron asignar los traslados.");}finally{setAssigning(false);}}
 async function reassign(id:string,field:"driver_user_id"|"mobilizer_user_id",value:string){const member=members.find(item=>item.user_id===value);const {error}=await supabase.from("mobility_transfers").update({[field]:value||null,[field==="driver_user_id"?"driver_name":"mobilizer_name"]:member?.profiles?.full_name||null}).eq("id",id).eq("organization_id",organization.id);setMessage(error?"No se pudo reasignar el traslado.":"Traslado reasignado.");if(!error)await load();}
 async function removeTransfer(item:MobilityTransfer){if(!window.confirm(`¿Borrar el traslado de ${item.voter_name}?`))return;const {error}=await supabase.from("mobility_transfers").delete().eq("id",item.id);if(error)return setMessage("No se pudo borrar el traslado.");if(item.driver_access_code){const route=await supabase.from("mobility_driver_routes").select("*").eq("id",item.driver_access_code).maybeSingle();if(route.data){const stops=((route.data as {stops?:{transfer_id:string}[]}).stops??[]).filter(stop=>stop.transfer_id!==item.id);await supabase.from("mobility_driver_routes").update({stops}).eq("id",item.driver_access_code);}}setSelectedTransfer(null);await load();setMessage("Traslado eliminado.");}
 async function saveTransfer(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!selectedTransfer)return;const data=new FormData(event.currentTarget),driverId=String(data.get("driver_user_id")||""),mobilizerId=String(data.get("mobilizer_user_id")||""),driver=members.find(member=>member.user_id===driverId),mobilizer=members.find(member=>member.user_id===mobilizerId);if(!driver)return setMessage("Elegí un chofer.");const {error}=await supabase.from("mobility_transfers").update({driver_user_id:driverId,driver_name:driver.profiles?.full_name||"Chofer",mobilizer_user_id:mobilizerId||null,mobilizer_name:mobilizer?.profiles?.full_name||null,vehicle_plate:String(data.get("vehicle_plate")||selectedTransfer.vehicle_plate||"")}).eq("id",selectedTransfer.id);if(error)return setMessage("No se pudo editar el traslado.");setEditing(false);setSelectedTransfer(null);await load();setMessage("Traslado actualizado.");}
 async function loadDemo(){if(!window.confirm("¿Cargar un operativo ficticio de movilidad?"))return;const driver=members.find(member=>member.active)?.user_id||user.id,driverName=members.find(member=>member.user_id===driver)?.profiles?.full_name||"Conductor demostración";let demoVoters=voters.slice(0,4);if(demoVoters.length<4){const base=["Clara Molina","Héctor Suárez","Elena Figueroa","Ricardo Luna"].map((full_name,index)=>({organization_id:organization.id,dni:`${33000111+index}`,full_name,address:["San Martín 820","Laprida 1450","Av. Belgrano 234","Córdoba 980"][index],circuit:"Demo",polling_place:"Escuela demostración",contact_status:"pendiente",assigned_to:null,source_data:{telefono:`38155500${index+1}`}}));const result=await supabase.from("voters").insert(base);demoVoters=[...demoVoters,...((result.data??[]) as Voter[])];}let destination=locations.find(location=>location.location_type==="escuela")||locations[0];if(!destination){const result=await supabase.from("headquarters").insert({organization_id:organization.id,name:"Escuela N° 101 · Demo",address:"Av. Sarmiento 1250",location_type:"escuela",active:true,latitude:-26.8165,longitude:-65.2161});destination=(result.data as Headquarters[])[0];}const statuses:["pendiente","buscada","en_destino","regresada"]=["pendiente","buscada","en_destino","regresada"],now=Date.now();for(let index=0;index<4;index++){const voter=demoVoters[index];if(!voter)continue;const id=crypto.randomUUID(),status=statuses[index],at=new Date(now-index*42*60000).toISOString(),latitude=-26.817-index*.006,longitude=-65.215+index*.005;await supabase.from("mobility_transfers").insert({id,organization_id:organization.id,operation_date:dateKey(new Date()),voter_id:String(voter.id),voter_dni:voter.dni,voter_name:voter.full_name,pickup_address:voter.address||"Dirección demo",voter_phone:(voter.source_data?.telefono||null),destination_id:String(destination?.id||""),destination_name:destination?.name||"Escuela demostración",destination_address:destination?.address||null,driver_user_id:driver,driver_name:driverName,mobilizer_user_id:null,vehicle_plate:"AE 2027 DE",status,status_updated_at:at,last_latitude:latitude,last_longitude:longitude,last_updated_by:driver,created_by:user.id});if(status!=="pendiente")await supabase.from("mobility_events").insert({organization_id:organization.id,transfer_id:id,voter_id:String(voter.id),driver_user_id:driver,actor_id:user.id,from_status:"pendiente",status,latitude,longitude,created_at:at});}setMessage("Operativo demo cargado. Abrí la subapp de movilidad para verlo como chofer.");await load();}
 ...
 const grouped=visible.reduce<Record<string,MobilityTransfer[]>>((groups,item)=>{const key=item.mobilizer_name||"Sin movilizador asignado";(groups[key]??=[]).push(item);return groups;},{});
 const delayed=items.filter(item=>item.status!=="regresada"&&item.status_updated_at&&Date.now()-new Date(item.status_updated_at).getTime()>30*60*1000);
 return <article className="panel mobility-operator">
   <PanelHead kicker="DÍA DE ELECCIONES" title="Operativo de movilidad" aside={<><button className="secondary compact" onClick={()=>void loadDemo()}>Cargar ejemplo</button><button className="primary compact" onClick={()=>setOpen(!open)}>＋ Armar jornada</button></>}/>
   <p className="form-message">Cada vehículo queda asociado a un chofer. Al iniciar sesión, el chofer ve solamente sus traslados de la jornada; los movilizadores acompañantes ven la misma lista.</p>
   <label className="mobility-day-selector">Jornada <input type="date" value={operationDate} onChange={event=>setOperationDate(event.target.value)}/></label>
   <div className="claim-summary"><article><b>{count("pendiente")}</b><span>Pendientes</span></article><article><b>{count("buscada")}</b><span>Buscadas</span></article><article><b>{count("en_destino")}</b><span>En destino</span></article><article><b>{count("regresada")}</b><span>Regresadas</span></article></div>
   {delayed.length>0&&<div className="info-banner">⚠ {delayed.length} traslado(s) llevan más de 30 minutos sin avanzar.</div>}
   <MobilityLiveMap points={items.map(item=>({id:item.id,name:item.voter_name,status:item.status,latitude:item.last_latitude,longitude:item.last_longitude,detail:`${item.driver_name||"Sin chofer"} · ${item.vehicle_plate||"Sin patente"} · ${statusCopy(item.status)}`}))}/>
   {open&&<form className="entry-form" onSubmit={assign}><div className="form-head"><div><p className="kicker">JORNADA POR VEHÍCULO</p><h2>Asignar hasta 20 personas</h2><p>Elegí el auto y su chofer una sola vez. Las personas seleccionadas aparecerán automáticamente en su celular.</p></div><button type="button" onClick={()=>setOpen(false)}>×</button></div><div className="form-grid">
     <label>Fecha del operativo<input name="operation_date" type="date" value={operationDate} onChange={event=>setOperationDate(event.target.value)} required/></label>
     <label>Vehículo<select required value={selectedVehicle} onChange={event=>{const value=event.target.value,vehicle=vehicles.find(item=>item.id===value);setSelectedVehicle(value);setSelectedDriver(vehicle?.responsible_user_id||"");}}><option value="">Elegir vehículo</option>{vehicles.filter(item=>item.record_type==="Vehículo").map(vehicle=><option key={vehicle.id} value={vehicle.id}>{vehicle.title} · {vehicle.contact_phone||"Patente a definir"}</option>)}</select></label>
     <label>Chofer del vehículo<select required value={selectedDriver} onChange={event=>setSelectedDriver(event.target.value)}><option value="">Elegir chofer</option>{members.filter(member=>member.active).map(member=><option key={member.user_id} value={member.user_id}>{member.profiles?.full_name||"Usuario"}</option>)}</select></label>
     <label>Patente<input name="vehicle_plate" defaultValue={vehicles.find(item=>item.id===selectedVehicle)?.contact_phone||""} placeholder="Ej.: AB 123 CD"/></label>
     <label>Movilizador acompañante<select name="mobilizer_user_id"><option value="">Sin movilizador</option>{members.filter(member=>member.active).map(member=><option key={member.user_id} value={member.user_id}>{member.profiles?.full_name||"Usuario"}</option>)}</select></label>
     <div className="wide mobility-voter-picker"><b>Personas del padrón · {selectedVoterIds.length}/20</b><input value={dniSearch} onChange={event=>setDniSearch(event.target.value.replace(/\D/g,""))} inputMode="numeric" placeholder="Escribí el DNI para buscar"/>{dniSearch.length>2&&<div className="mobility-voter-options">{(()=>{const matches=voters.filter(voter=>voter.dni.replace(/\D/g,"").includes(dniSearch));const available=matches.filter(voter=>!items.some(item=>item.voter_id===String(voter.id)));if(!matches.length)return <small>No se encontró ese DNI en el padrón cargado. Revisá el número o importá el padrón.</small>;if(!available.length){const existing=items.find(item=>item.voter_id===String(matches[0]?.id));return <button type="button" className="mobility-existing" onClick={()=>existing&&setSelectedTransfer(existing)}>Esta persona ya tiene traslado. Ver / editar traslado →</button>;}return available.slice(0,20).map(voter=>{const checked=selectedVoterIds.includes(String(voter.id));return <label key={voter.id}><input type="checkbox" checked={checked} disabled={!checked&&selectedVoterIds.length>=20} onChange={()=>setSelectedVoterIds(current=>checked?current.filter(id=>id!==String(voter.id)):[...current,String(voter.id)])}/><span><strong>{voter.full_name}</strong><small>DNI {voter.dni} · {voter.address||"Sin domicilio"} · {voter.polling_place||"Escuela a confirmar"}</small></span></label>;});})()}</div>}<small>La escuela se toma automáticamente del padrón de cada persona.</small></div>
   </div><div className="form-actions"><button type="button" onClick={()=>setOpen(false)}>Cancelar</button><button className="primary compact" disabled={assigning}>{assigning?"Guardando jornada…":"Asignar jornada"}</button></div></form>}
   <div className="module-actions"><button className={filter==="todos"?"secondary compact":"text-button"} onClick={()=>setFilter("todos")}>Todos {items.length}</button>{(["pendiente","buscada","en_destino","regresada"] as const).map(status=><button key={status} className={filter===status?"secondary compact":"text-button"} onClick={()=>setFilter(status)}>{statusCopy(status)} {count(status)}</button>)}</div>
   {visible.length===0?<Empty title="Sin traslados para esta jornada" text="Elegí el auto, su chofer y hasta 20 personas del padrón para crear la primera hoja de ruta."/>:<div className="mobility-groups">{Object.entries(grouped).map(([mobilizer,transfers])=>{const code=transfers.find(item=>item.driver_access_code)?.driver_access_code;return <details className="panel mobility-group" key={mobilizer}><summary><span><p className="kicker">MOVILIZADOR</p><strong>{mobilizer}</strong><small>{transfers.length} persona(s) a trasladar</small></span><b>Ver personas ▾</b></summary><div className="mobility-group-tools">{code&&<><code>{code}</code><button type="button" className="secondary compact" onClick={()=>void navigator.clipboard.writeText(`Abrí https://eleccionestucuman.web.app/movilidad y pegá este código: ${code}`).then(()=>setMessage("Código copiado. Ya podés pegarlo en WhatsApp.")).catch(()=>setMessage("No se pudo copiar automáticamente. Seleccioná el código y copialo."))}>Copiar código para WhatsApp</button></>}</div><div className="campaign-record-list">{transfers.map(item=><button type="button" key={item.id} className="mobility-transfer-row" onClick={()=>{setSelectedTransfer(item);setEditing(false);}}><span className={`priority-dot ${item.status==="regresada"?"baja":item.status==="buscada"?"alta":"media"}`}/><span><strong>{item.voter_name}</strong><small>{item.destination_name}</small></span><em>{statusCopy(item.status)}</em><b>Ver detalle →</b></button>)}</div></details>;})}</div>}
   {selectedTransfer&&typeof document!=="undefined"&&createPortal(<div className="mobility-modal-backdrop" role="presentation"><section className="mobility-modal panel" role="dialog" aria-modal="true"><div className="form-head"><div><p className="kicker">DETALLE DEL TRASLADO</p><h2>{selectedTransfer.voter_name}</h2></div><button type="button" onClick={()=>setSelectedTransfer(null)}>×</button></div>{editing?<form className="entry-form" onSubmit={saveTransfer}><div className="form-grid"><label>Chofer<select name="driver_user_id" defaultValue={selectedTransfer.driver_user_id}>{members.filter(member=>member.active).map(member=><option key={member.user_id} value={member.user_id}>{member.profiles?.full_name||"Usuario"}</option>)}</select></label><label>Movilizador<select name="mobilizer_user_id" defaultValue={selectedTransfer.mobilizer_user_id||""}><option value="">Sin movilizador</option>{members.filter(member=>member.active).map(member=><option key={member.user_id} value={member.user_id}>{member.profiles?.full_name||"Usuario"}</option>)}</select></label><label>Patente<input name="vehicle_plate" defaultValue={selectedTransfer.vehicle_plate||""}/></label><label>Destino<select name="destination_id" defaultValue={locations.find(location=>location.name===selectedTransfer.destination_name)?.id||""}>{locations.filter(location=>location.location_type==="escuela"||location.location_type==="sede").map(location=><option key={location.id} value={location.id}>{location.name}</option>)}</select></label></div><div className="form-actions"><button type="button" onClick={()=>setEditing(false)}>Cancelar</button><button className="primary compact">Guardar cambios</button></div></form>:<><p><b>DNI:</b> {voters.find(voter=>String(voter.id)===selectedTransfer.voter_id)?.dni||"No disponible"}</p><p><b>Domicilio:</b> {selectedTransfer.pickup_address||"Sin domicilio"}</p><p><b>Destino:</b> {selectedTransfer.destination_name}</p><p><b>Chofer:</b> {selectedTransfer.driver_name||"Sin asignar"}</p><p><b>Movilizador:</b> {selectedTransfer.mobilizer_name||"Sin asignar"}</p><p><b>Estado:</b> {statusCopy(selectedTransfer.status)}</p><div className="form-actions"><button type="button" onClick={()=>setEditing(true)}>Editar traslado</button><button type="button" className="danger-button" onClick={()=>void removeTransfer(selectedTransfer)}>Borrar traslado</button></div></>}</section></div>,document.body)}
   {message&&<button className="toast" onClick={()=>setMessage("")}>{message}<span>×</span></button>}
 </article>;
}
...
function CampaignModuleView({moduleId,user,organization,members,locations,items,voters,reload}:{moduleId:keyof typeof campaignModuleConfig;user:User;organization:Organization;members:Member[];locations:Headquarters[];items:CampaignRecord[];voters:Voter[];reload:()=>Promise<void>}){
  const config=campaignModuleConfig[moduleId],[open,setOpen]=useState(false),[attendanceOpen,setAttendanceOpen]=useState(false),[message,setMessage]=useState(""),[attendance,setAttendance]=useState<EventAttendance[]>([]),[editingRecord,setEditingRecord]=useState<CampaignRecord|null>(null);
  ...
  async function removeRecord(item:CampaignRecord){if(!window.confirm(`¿Borrar ${item.title}?`))return;const {error}=await supabase.from(config.collection).delete().eq("id",item.id).eq("organization_id",organization.id);if(error)setMessage("No se pudo borrar el registro.");else{setEditingRecord(null);await reload();setMessage("Registro eliminado.");}}
  async function saveRecord(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!editingRecord)return;const data=new FormData(event.currentTarget);const {error}=await supabase.from(config.collection).update({title:data.get("title"),responsible_user_id:data.get("responsible_user_id")||null,contact_phone:data.get("contact_phone")||null,status:data.get("status")}).eq("id",editingRecord.id);if(error)setMessage("No se pudo guardar el cambio.");else{setEditingRecord(null);await reload();setMessage("Registro actualizado.");}}
  ...
  return <section><ModuleTitle kicker={config.kicker} title={config.title} subtitle={config.subtitle}><div className="module-actions"><PdfButton organization={organization} title={titleForPdf} columns={["Registro","Tipo","Ubicación","Estado","Responsable"]} rows={items.map(item=>[item.title,item.record_type||"-",item.location||"-",item.status,members.find(member=>member.user_id===item.responsible_user_id)?.profiles?.full_name||"-"])}/><BulkTemplateButton file={`modelo-${moduleId}.csv`} label="Excel"/>{moduleId==="eventos"&&<button className="secondary compact" onClick={()=>setAttendanceOpen(!attendanceOpen)}>＋ Asistencia</button>}<button className="primary compact" onClick={()=>setOpen(!open)}>＋ Nuevo</button></div></ModuleTitle>
    ...
    {moduleId==="logistica"&&<MobilityOperator organization={organization} members={members} locations={locations} voters={voters} vehicles={items} user={user}/>}
    ...
    <article className="panel"><PanelHead kicker="SEGUIMIENTO" title={config.title} aside={`${items.length} registros`}/>{items.length===0?<Empty title={`Sin ${config.singular}s cargados`} text="Usá el botón Nuevo para crear el primer registro."/>:<div className="campaign-record-list">{items.map(item=><div className={moduleId==="logistica"?"logistics-record-row":""} key={item.id}><span className={`priority-dot ${item.priority}`}/><div><strong>{item.title}</strong><small>{item.record_type||config.singular} · {item.location||locations.find(location=>String(location.id)===item.linked_location_id)?.name||"Sin ubicación"} · {item.scheduled_for?new Date(item.scheduled_for).toLocaleString("es-AR",{day:"2-digit",month:"short",hour:"2-digit"}):"Sin fecha"}</small></div><em>{members.find(member=>member.user_id===item.responsible_user_id)?.profiles?.full_name||item.contact_name||"Sin responsable"}</em><select value={item.status} onChange={event=>changeStatus(item.id,event.target.value)}><option value="pendiente">Pendiente</option><option value="en_proceso">En proceso</option><option value="confirmado">Confirmado</option><option value="completado">Completado</option></select>{moduleId==="logistica"&&<><button type="button" className="text-button logistics-edit" onClick={()=>setEditingRecord(item)}>Editar</button><button type="button" className="text-button danger-text logistics-delete" onClick={()=>void removeRecord(item)}>Borrar</button></>}</div>)}</div>}</article>
    {editingRecord&&typeof document!=="undefined"&&createPortal(<div className="mobility-modal-backdrop"><section className="mobility-modal panel"><div className="form-head"><div><p className="kicker">EDITAR VEHÍCULO / RECURSO</p><h2>{editingRecord.title}</h2></div><button type="button" onClick={()=>setEditingRecord(null)}>×</button></div><form className="entry-form" onSubmit={saveRecord}><div className="form-grid"><label>Nombre<input name="title" defaultValue={editingRecord.title} required/></label><label>Estado<select name="status" defaultValue={editingRecord.status}><option value="pendiente">Pendiente</option><option value="en_proceso">En proceso</option><option value="confirmado">Confirmado</option><option value="completado">Completado</option></select></label><label>Chofer o responsable<select name="responsible_user_id" defaultValue={editingRecord.responsible_user_id||""}><option value="">Sin asignar</option>{members.filter(member=>member.active).map(member=><option key={member.user_id} value={member.user_id}>{member.profiles?.full_name||"Usuario"}</option>)}</select></label><label>Patente / teléfono<input name="contact_phone" defaultValue={editingRecord.contact_phone||""}/></label></div><div className="form-actions"><button type="button" onClick={()=>setEditingRecord(null)}>Cancelar</button><button className="primary compact">Guardar</button><button type="button" className="danger-button" onClick={()=>void removeRecord(editingRecord)}>Borrar</button></div></form></section></div>,document.body)}
    {message&&<button className="toast" onClick={()=>setMessage("")}>{message}<span>×</span></button>}
  </section>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if ("serviceWorker" in navigator) {
      void navigator.serviceWorker.register("/sw.js?v=6");
    }
