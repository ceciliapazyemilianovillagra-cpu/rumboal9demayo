...
function MobilityOperator({organization,members,locations,voters,vehicles,user}:{organization:Organization;members:Member[];locations:Headquarters[];voters:Voter[];vehicles:CampaignRecord[];user:User}){
 const [items,setItems]=useState<MobilityTransfer[]>([]),[events,setEvents]=useState<{id:string;transfer_id:string;status:string;created_at:string;actor_id:string}[]>([]),[open,setOpen]=useState(false),[message,setMessage]=useState(""),[filter,setFilter]=useState("todos"),[operationDate,setOperationDate]=useState(dateKey(new Date())),[selectedVehicle,setSelectedVehicle]=useState(""),[selectedDriver,setSelectedDriver]=useState("");
 const load=useCallback(async()=>{const [result,eventResult]=await Promise.all([supabase.from("mobility_transfers").select("*").eq("organization_id",organization.id).eq("operation_date",operationDate).order("status_updated_at",{ascending:false}),supabase.from("mobility_events").select("*").eq("organization_id",organization.id).order("created_at",{ascending:false}).limit(200)]);setItems(result.error?[]:(result.data??[]) as MobilityTransfer[]);setEvents(eventResult.error?[]:(eventResult.data??[]) as {id:string;transfer_id:string;status:string;created_at:string;actor_id:string}[]);},[organization.id,operationDate]);
 useEffect(()=>{void load();},[load]);
 async function assign(event:FormEvent<HTMLFormElement>){event.preventDefault();const form=event.currentTarget,data=new FormData(form),selectedIds=data.getAll("voter_id").map(String),driver=members.find(item=>item.user_id===selectedDriver),destination=locations.find(item=>String(item.id)===String(data.get("destination_id"))),vehicle=vehicles.find(item=>item.id===selectedVehicle);if(!selectedIds.length||selectedIds.length>20||!driver||!destination||!vehicle)return setMessage("Elegí un vehículo, su chofer, destino y entre 1 y 20 personas.");const selectedVoters=selectedIds.map(id=>voters.find(item=>String(item.id)===id)).filter(Boolean) as Voter[];if(selectedVoters.length!==selectedIds.length)return setMessage("Una de las personas elegidas no está disponible.");const operationDate=String(data.get("operation_date")),now=new Date().toISOString(),plate=String(data.get("vehicle_plate")||vehicle.contact_phone||"Patente a definir");const records=selectedVoters.map(voter=>({id:crypto.randomUUID(),organization_id:organization.id,operation_date:operationDate,voter_id:String(voter.id),voter_dni:voter.dni,voter_name:voter.full_name,pickup_address:voter.address||"Dirección a confirmar",voter_phone:(voter.source_data?.telefono||voter.source_data?.phone||null),destination_id:String(destination.id),destination_name:destination.name,destination_address:destination.address||null,driver_user_id:driver.user_id,driver_name:driver.profiles?.full_name||"Chofer",mobilizer_user_id:data.get("mobilizer_user_id")||null,mobilizer_name:members.find(item=>item.user_id===data.get("mobilizer_user_id"))?.profiles?.full_name||null,vehicle_id:vehicle.id,vehicle_plate:plate,status:"pendiente",status_updated_at:now,created_by:user.id}));const {error}=await supabase.from("mobility_transfers").insert(records);if(error)setMessage("No se pudieron asignar los traslados. Verificá los permisos y volvé a intentar.");else{await supabase.from("logistics_items").update({responsible_user_id:driver.user_id,contact_phone:plate}).eq("id",vehicle.id);form.reset();setSelectedVehicle("");setSelectedDriver("");setOpen(false);setMessage(`${records.length} traslado(s) asignados: al ingresar, ${driver.profiles?.full_name||"el chofer"} los verá automáticamente en Mi movilidad.`);await load();}}
 ...
 return <article className="panel mobility-operator">
   ...
   {open&&<form className="entry-form" onSubmit={assign}><div className="form-head"><div><p className="kicker">JORNADA POR VEHÍCULO</p><h2>Asignar hasta 20 personas</h2><p>Elegí el auto y su chofer una sola vez. Las personas seleccionadas aparecerán automáticamente en su celular.</p></div><button type="button" onClick={()=>setOpen(false)}>×</button></div><div className="form-grid">
     ...
     <label className="wide">Personas del padrón (máximo 20)<select name="voter_id" required multiple size={9}>{voters.filter(voter=>!items.some(item=>item.voter_id===String(voter.id))).map(voter=><option key={voter.id} value={voter.id}>{voter.full_name} · DNI {voter.dni} · {voter.address||"Sin domicilio"}</option>)}</select><small>Mantené Ctrl (PC) o tocá las filas en celular para seleccionar varias personas.</small></label>
   </div><div className="form-actions"><button type="button" onClick={()=>setOpen(false)}>Cancelar</button><button className="primary compact">Asignar jornada</button></div></form>}
