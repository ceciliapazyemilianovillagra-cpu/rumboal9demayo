...
function MobilityOperator({organization,members,locations,voters,vehicles,user}:{organization:Organization;members:Member[];locations:Headquarters[];voters:Voter[];vehicles:CampaignRecord[];user:User}){
 const [items,setItems]=useState<MobilityTransfer[]>([]),[events,setEvents]=useState<{id:string;transfer_id:string;status:string;created_at:string;actor_id:string}[]>([]),[open,setOpen]=useState(false),[message,setMessage]=useState(""),[filter,setFilter]=useState("todos");
 const load=useCallback(async()=>{const [result,eventResult]=await Promise.all([supabase.from("mobility_transfers").select("*").eq("organization_id",organization.id).order("status_updated_at",{ascending:false}),supabase.from("mobility_events").select("*").eq("organization_id",organization.id).order("created_at",{ascending:false}).limit(200)]);setItems(result.error?[]:(result.data??[]) as MobilityTransfer[]);setEvents(eventResult.error?[]:(eventResult.data??[]) as {id:string;transfer_id:string;status:string;created_at:string;actor_id:string}[]);},[organization.id]);
