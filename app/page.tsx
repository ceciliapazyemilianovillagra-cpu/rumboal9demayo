...
function MobilityOperator({organization,members,locations,voters,vehicles,user}:{organization:Organization;members:Member[];locations:Headquarters[];voters:Voter[];vehicles:CampaignRecord[];user:User}){
 ...
 return <article className="panel mobility-operator">
   ...
   {visible.length===0?<Empty title="Sin traslados para esta jornada" text="Elegí el auto, su chofer y hasta 20 personas del padrón para crear la primera hoja de ruta."/>:<div className="campaign-record-list">{visible.map(item=><div key={item.id}><span className={`priority-dot ${item.status==="regresada"?"baja":item.status==="buscada"?"alta":"media"}`}/><div><strong>{item.voter_name}</strong><small>{item.driver_name||"Sin chofer"} · {item.vehicle_plate||"Sin patente"} · {item.destination_name}</small></div><em>{statusCopy(item.status)}</em><select value={item.driver_user_id} onChange={event=>void reassign(item.id,"driver_user_id",event.target.value)}>{members.filter(member=>member.active).map(member=><option value={member.user_id} key={member.user_id}>{member.profiles?.full_name||"Chofer"}</option>)}</select></div>)}</div>}
