...
function MobilityOperator({organization,members,locations,voters,vehicles,user}:{organization:Organization;members:Member[];locations:Headquarters[];voters:Voter[];vehicles:CampaignRecord[];user:User}){
 ...
 return <article className="panel mobility-operator">
   ...
   {open&&<form className="entry-form" onSubmit={assign}><div className="form-head"><div><p className="kicker">JORNADA POR VEHÍCULO</p><h2>Asignar hasta 20 personas</h2><p>Elegí el auto y su chofer una sola vez. Las personas seleccionadas aparecerán automáticamente en su celular.</p></div><button type="button" onClick={()=>setOpen(false)}>×</button></div><div className="form-grid">
     ...
     <div className="wide mobility-voter-picker"><b>Personas del padrón · {selectedVoterIds.length}/20</b><input value={dniSearch} onChange={event=>setDniSearch(event.target.value.replace(/\D/g,""))} inputMode="numeric" placeholder="Escribí el DNI para buscar"/>{dniSearch.length>2&&<div className="mobility-voter-options">{voters.filter(voter=>voter.dni.includes(dniSearch)&&!items.some(item=>item.voter_id===String(voter.id))).slice(0,20).map(voter=>{const checked=selectedVoterIds.includes(String(voter.id));return <label key={voter.id}><input type="checkbox" checked={checked} disabled={!checked&&selectedVoterIds.length>=20} onChange={()=>setSelectedVoterIds(current=>checked?current.filter(id=>id!==String(voter.id)):[...current,String(voter.id)])}/><span><strong>{voter.full_name}</strong><small>DNI {voter.dni} · {voter.address||"Sin domicilio"}</small></span></label>;})}{!voters.some(voter=>voter.dni.includes(dniSearch))&&<small>No se encontró ese DNI.</small>}</div>}<small>Escribí un DNI, marcá la persona y seguí con el siguiente.</small></div>
   ...
   {visible.length===0?<Empty title="Sin traslados para esta jornada" text="Elegí el auto, su chofer y hasta 20 personas del padrón para crear la primera hoja de ruta."/>:<div className="campaign-record-list">{visible.map(item=><div key={item.id}><span className={`priority-dot ${item.status==="regresada"?"baja":item.status==="buscada"?"alta":"media"}`}/><div><strong>{item.voter_name}</strong><small>{item.driver_name||"Sin chofer"} · {item.vehicle_plate||"Sin patente"} · {item.destination_name}{item.driver_access_code?` · Código ${item.driver_access_code}`:""}</small></div><em>{statusCopy(item.status)}</em><select value={item.driver_user_id} onChange={event=>void reassign(item.id,"driver_user_id",event.target.value)}>{members.filter(member=>member.active).map(member=><option value={member.user_id} key={member.user_id}>{member.profiles?.full_name||"Chofer"}</option>)}</select></div>)}</div>}
