...
function MobilityOperator({organization,members,locations,voters,vehicles,user}:{organization:Organization;members:Member[];locations:Headquarters[];voters:Voter[];vehicles:CampaignRecord[];user:User}){
 ...
 async function saveTransfer(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!selectedTransfer)return;const data=new FormData(event.currentTarget),driverId=String(data.get("driver_user_id")||""),mobilizerId=String(data.get("mobilizer_user_id")||""),destination=locations.find(location=>String(location.id)===String(data.get("destination_id"))),driver=members.find(member=>member.user_id===driverId),mobilizer=members.find(member=>member.user_id===mobilizerId);if(!driver||!destination)return setMessage("Elegí chofer y destino.");const {error}=await supabase.from("mobility_transfers").update({driver_user_id:driverId,driver_name:driver.profiles?.full_name||"Chofer",mobilizer_user_id:mobilizerId||null,mobilizer_name:mobilizer?.profiles?.full_name||null,destination_id:String(destination.id),destination_name:destination.name,destination_address:destination.address||null,vehicle_plate:String(data.get("vehicle_plate")||selectedTransfer.vehicle_plate||"")}).eq("id",selectedTransfer.id);if(error)return setMessage("No se pudo editar el traslado.");setEditing(false);setSelectedTransfer(null);await load();setMessage("Traslado actualizado.");}
 ...
 return <article className="panel mobility-operator">
   ...
   {open&&<form className="entry-form" onSubmit={assign}><div className="form-head"><div><p className="kicker">JORNADA POR VEHÍCULO</p><h2>Asignar hasta 20 personas</h2><p>Elegí el auto y su chofer una sola vez. Las personas seleccionadas aparecerán automáticamente en su celular.</p></div><button type="button" onClick={()=>setOpen(false)}>×</button></div><div className="form-grid">
     ...
     <label>Escuela / institución de destino<select name="destination_id" required><option value="">Elegir destino</option>{locations.filter(location=>location.location_type==="escuela"||location.location_type==="sede").map(location=><option key={location.id} value={location.id}>{location.name} · {location.address}</option>)}</select></label>
     <div className="wide mobility-voter-picker"><b>Personas del padrón · {selectedVoterIds.length}/20</b><input value={dniSearch} onChange={event=>setDniSearch(event.target.value.replace(/\D/g,""))} inputMode="numeric" placeholder="Escribí el DNI para buscar"/>{dniSearch.length>2&&<div className="mobility-voter-options">{(()=>{const matches=voters.filter(voter=>voter.dni.replace(/\D/g,"").includes(dniSearch));const available=matches.filter(voter=>!items.some(item=>item.voter_id===String(voter.id)));if(!matches.length)return <small>No se encontró ese DNI en el padrón cargado. Revisá el número o importá el padrón.</small>;if(!available.length)return <small>Esta persona ya tiene un traslado asignado para esta jornada.</small>;return available.slice(0,20).map(voter=>{const checked=selectedVoterIds.includes(String(voter.id));return <label key={voter.id}><input type="checkbox" checked={checked} disabled={!checked&&selectedVoterIds.length>=20} onChange={()=>setSelectedVoterIds(current=>checked?current.filter(id=>id!==String(voter.id)):[...current,String(voter.id)])}/><span><strong>{voter.full_name}</strong><small>DNI {voter.dni} · {voter.address||"Sin domicilio"}</small></span></label>;});})()}</div>}<small>Escribí un DNI, marcá la persona y seguí con el siguiente.</small></div>
   ...
   <article className="mobility-history"><PanelHead kicker="AUDITORÍA" title="Últimos movimientos" aside={`${events.length} registros`}/>{events.length===0?<p>Los cambios del chofer aparecerán aquí con hora y estado.</p>:events.slice(0,12).map(event=><p key={event.id}><b>{statusCopy(event.status)}</b> · {items.find(item=>item.id===event.transfer_id)?.voter_name||"Traslado"} · {new Date(event.created_at).toLocaleString("es-AR")}</p>)}</article>{message&&<button className="toast" onClick={()=>setMessage("")}>{message}<span>×</span></button>}
 </article>;
}
